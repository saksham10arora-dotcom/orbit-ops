...
app = FastAPI(title="OrbitAir API")
...
@app.get("/")
def root():
    return {"status": "OrbitAir API running"}


@app.get("/api/city")
async def get_city_data(name: str = Query(..., description="City name")):
    ...
    # Build hourly forecast (next 48 h only)
    ...
    for i, t in enumerate(times):
        try:
            ...
            elif dt < now and len(historical) < 168:  # 7 days
    ...
    return {
        ...
        "forecast": forecast,
        "historical": historical[-48:],  # last 48h for trends
    }
